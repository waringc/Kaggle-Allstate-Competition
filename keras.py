...
Used neural networks to generate test and out of fold train predictions for the
...
f_off = [f for f in tr_te.columns if 'OOF' in f]
scaler = StandardScaler()
tmp2 = csr_matrix(scaler.fit_transform(tr_te[f_off]))
sparse_data.append(tmp2)
...
## train predictions
df1 = pd.DataFrame({'id': id_train, 'loss': pred_oob})
df1.to_csv('Keras-preds_oob_Stacker3.csv', index=False)


## test predictions
pred_test /= (nfolds * nbags)
df2 = pd.DataFrame({'id': id_test, 'loss': pred_test})
df2.to_csv('submission_keras_shift_perm_Stacker3.csv', index=False)

df1.to_csv('OOF_Predictions.csv', index=False)
df2.to_csv('Test_Predictions.csv', index=False)